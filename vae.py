...
class VAE(TrainableLayer):
    ...
    def __init__(self,
        ...
        self.conv_output_channels = [32, 64, 128, 256, 512]
        self.conv_kernel_sizes = [5, 4, 4, 4, 4]
        self.conv_pooling_factors = [1, 1, 1, 1, 1]
        self.conv_kernel_stride = [1, 2, 1, 2, 1]
        self.acti_func_conv = ['leakyrelu', 'leakyrelu', 'leakyrelu', 'leakyrelu', 'leakyrelu']
        ...
        self.layer_sizes_encoder = [512, 128]
        self.acti_func_encoder = ['leakyrelu', 'leakyrelu']
        self.number_of_latent_variables = 64
        self.number_of_samples_from_posterior = 100
        self.layer_sizes_decoder_shared = []
        ...
        self.trans_conv_output_channels_means = [256, 128, 64, 32, 32, 32, 3]
        self.trans_conv_kernel_sizes_means = [4, 4, 4, 4, 5, 1, 1, 1]
        self.trans_conv_kernel_stride_means =  [1, 2, 1, 2, 1, 1, 1, 1]
        self.trans_conv_unpooling_factors_means = [1, 2, 1, 2, 1, 2, 1, 1]
        self.acti_func_trans_conv_means = ['leakyrelu', 'leakyrelu', 'leakyrelu', 'leakyrelu', 'leakyrelu', 'leakyrelu', 'sigmoid', None]
    ...
    def layer_op(self, images, is_training=True, **unused_kwargs):
        ...
        sample = tf.expand_dims(tf.expand_dims(tf.expand_dims(sample, 1), 1), 1)
...
class ConvEncoder(TrainableLayer):
    ...
    """
    # TODO: mettre maxout function dans niftinet.layer.activation script
    def __init__(self,
                 denoising_variance,
...
class ConvDecoder(TrainableLayer):
    ...
    def __init__(self,
        ...
        self.trans_conv_kernel_stride = trans_conv_kernel_stride
    ...
    def layer_op(self, codes, is_training):
        ...
        decoders_fc = []
        if len(self.acti_func_decoder) > 0:
        ...
        for i in range(0, len(self.trans_conv_output_channels)):
            ...
            decoders_cnn.append(DeconvolutionalLayer(
                ...
                stride=self.trans_conv_kernel_stride[i],
                padding='SAME',
                with_bias=True,
                feature_normalization='batch',
        ...
        for i in range(0, len(self.trans_conv_output_channels)):
            if self.upsampling_mode == 'DECONV':
                ...
                print("Deconv shape ", i, ": ", flow.shape)
...
def clip(x, logvars_lower_bound, logvars_upper_bound):
    return tf.clip_by_value(x, logvars_lower_bound, logvars_upper_bound)
